                                     ...
                                     const int class_number)
{
    ...
    //
    const int vxsize = 512; //(pxmax - pxmin) / voxel_size[0] +1;
    const int vysize = 512; //(pymax - pymin) / voxel_size[1] +1;
    ...
    for (int i=0; i< original_points.rows(); ++i) {
        ...
        const int pt_class = original_points(i,5);
