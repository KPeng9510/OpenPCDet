...
def seg_concat():
    ...
    for my_sample in nusc.sample:
        ...
        #print(ori_filename)
        anno_data = torch.from_numpy(np.float32(np.fromfile("/mrtstorage/users/kpeng/nu_lidar_seg/processed_anno/"+sample_data_token+"_lidarseg.bin", dtype=np.uint8, count=-1)))
